...
const SellStocks = ({handleClose, selectedAsset, setUpdateBalanceFlag}) => {
  ...
  const handleSellStocks = async (userStock) => {
    try {
      ...
      const sellResponse = await sellStocks({
        user_id,
        quantity: { quantity: parseFloat(quantity), price: parseFloat(price) },
        symbol: symbol,
      });
  ...
  const paginatedTransactions = transactions.user_stocks
  ? transactions.user_stocks
      .filter((transaction) => 
        transaction.symbol === selectedAsset && parseFloat(transaction.quantity) > 0
      )
      .slice(
            ...
            <div className="flex justify-center">
              ...
              <div className="flex justify-center w-full  mt-4 mb-4">
                ...
                <button
                  ...
                  disabled={
                    paginatedTransactions.length < transactionsPerPage
                  }
                >
                  Next
                </button>
